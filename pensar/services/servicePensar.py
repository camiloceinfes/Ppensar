...
class Ppensar():
    ...
    def calculate_prueba_estudiantes(self, codigoColegio, anio, idPrueba, grado, db):
        ...
        try:
            ...
            query = text(f"EXEC {procedure_name} @Codigo=:Codigo, @ANNOA=:ANNOA, @IDPRUEBA=:IDPRUEBA, @Grado=:Grado")
            result = db.execute(query, {"Codigo": codigoColegio, "ANNOA": anio, "IDPRUEBA": idPrueba, "Grado": grado}).fetchall()
